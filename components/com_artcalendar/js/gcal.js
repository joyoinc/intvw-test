	...
	$.fullCalendar.gcalFeed = function(feedUrl, options) {
		...
		return function(start, end, callback) {
			if ((feedUrl.indexOf(',')) > -1) {
				window.ac_events = [];
				var urls = feedUrl.split(',');
				var uCount = urls.length;
				...
				function(data) {
					if (data.feed.entry)
						$.each(data.feed.entry, function(i, entry) {
							...
							var showTime = options.showTime && entry['gd$when'][0]['startTime'].indexOf('T') != -1;
							window.ac_events.push({
